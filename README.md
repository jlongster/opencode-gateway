...
On the home screen, `/cd` autocomplete lists gateway Images as virtual directories. The built-in Image is named `default`; selecting a named Image creates the next workspace from that Modal filesystem snapshot. Inside a workspace, filesystem requests continue to route to its Modal VM.
