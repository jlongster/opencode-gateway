...
The default sandbox image installs `@opencode-ai/cli@dev` globally with Bun. Override the base image with `--image` or pin another CLI version with `--opencode-version`. At sandbox startup, the large standalone binary is copied from Modal's image filesystem to writable `/tmp` before execution; this avoids a gVisor lazy-read issue with embedded Bun chunks. The gateway asks a temporary `opencode2` process to initialize the database, then imports credentials with SQLite. Set `OPENCODE_GATEWAY_UPSTREAM_PASSWORD` when the sandbox OpenCode password should differ from the gateway password.
