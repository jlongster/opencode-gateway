...
The default sandbox image installs `@opencode-ai/cli@dev` globally with Bun. Override the base image with `--image` or pin another CLI version with `--opencode-version`. Sandboxes use Modal's full VM runtime so Bun can reliably read embedded chunks from the standalone executable. The gateway asks a temporary `opencode2` process to initialize the database, then imports credentials with SQLite. Set `OPENCODE_GATEWAY_UPSTREAM_PASSWORD` when the sandbox OpenCode password should differ from the gateway password.
